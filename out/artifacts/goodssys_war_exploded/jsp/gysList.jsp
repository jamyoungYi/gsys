...
<html>
  <head>
    ...
    <title>My JSP 'gysList.jsp' starting page</title>
  ...
  <body>
	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
		  <tr>
		    <td width="99%" align="left" valign="top">您的位置：供应商管理</td>
		  </tr>
		  <tr>
			    ...
			    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
			  		<tr>
			   			<td width="90%" align="left" valign="middle">
					         <form method="post" action="<%=path%>/supplier/findByNameAndByContactSupplierLike">
						         <span>名称：</span>
						         <input type="text" name="name" value="" class="text-word">
						         <span>联系人：</span>
						         <input type="text" name="contacts" value="" class="text-word">
						         <input name="" type="submit" value="查询" class="text-but">
					         </form>
			         	</td>
			  		 	<td width="10%" align="center" valign="middle" style="text-align:right; width:150px;"><a href="<%=path %>/jsp/gysAdd.jsp" target="main" onFocus="this.blur()" class="add">新增供应商</a></td>
			  		</tr>
				</table>
		    </td>
		  </tr>
					...
					<c:forEach items="${supplierList}" var="supplier" varStatus="status">
						<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
							...
							<td align="center" valign="middle" class="borderbottom">
								<a href="<%=path%>/supplier/toUpdateCustomer?id=${supplier.s_id}" target="main" onFocus="this.blur()" class="add">编辑</a><span class="gray">&nbsp;|&nbsp;</span>
								<a href="<%=path%>/supplier/deleteSupplier?id=${supplier.s_id}" target="main" onFocus="this.blur()" class="add">删除</a>
							</td>
						</tr>
					</c:forEach>
	  	...
	  	<tr>
	  		<td colspan="6" align="left" valign="top" class="fenye">
					...
					<a href="<%=path %>/gys_queryAll" target="main" onFocus="this.blur()">首页</a>  
					<%--   上一页    当前页  -1    只有  当前页 大于 1 时 才可以 -1 --%>
					<c:choose>
						<c:when test="${page.pageNow>1 }">
							<a href="<%=path %>/gys_queryAll?pageNow=${page.pageNow-1}" target="main" onFocus="this.blur()">上一页</a>
						</c:when>
						<c:otherwise>
  							上一页
  						</c:otherwise>
					...
					<c:forEach begin="${begin}" end="${end}" var="i">
						<a href="<%=path %>/gys_queryAll?pageNow=${i}" target="main" onFocus="this.blur()"><span class="current">${i }</span></a>
					</c:forEach>
					<c:choose>
						<%-- 下一页  当前页 +1  只有  当前页 小于 总页数 时 才可以加1 --%>
						<c:when test="${page.pageNow  <page.pageCount }">
							<a href="<%=path %>/gys_queryAll?pageNow=${page.pageNow+1}" target="main" onFocus="this.blur()">下一页</a>
						</c:when>
						<c:otherwise>
							下一页 
						</c:otherwise>
					</c:choose> 
					<a href="<%=path %>/gys_queryAll?pageNow=${page.pageCount}" target="main" onFocus="this.blur()">尾页</a>
				</td>
	  	</tr>
	</table>
  </body>
